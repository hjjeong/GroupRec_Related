...
def test_one_group(x, model, Ks, group_trainRatings, group_testRatings, num_items):
    #rating = x[0]
    g = x #group 1개 들어옴
    ...
    test_items = list(all_items-set(training_items)) #modified
    #test_items = list(all_items) #modified
    ...
    #rating, _ = model(group_var, item_var, is_training=True)
    #rating = rating.cpu().data.numpy()
...
def test(model, groups_to_test, Ks, group_trainRatings, num_items, drop_flag=False, batch_test_flag = False):
    ...
    for g_id in test_groups:
        re = test_one_group(g_id, model, Ks, group_trainRatings, test_groups, num_items)
