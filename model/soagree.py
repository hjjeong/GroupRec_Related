...
class SoAGREE(nn.Module):
    ...
    def forward(self, group_inputs, item_inputs, user_inputs=None):
        # train group
    ...
    def grp_forward(self, group_inputss, item_inputss):
        ...
        item_embeds_full = self.itemembeds(Variable(item_inputss).cuda())
    ...
    '''
    def user_aggregate(self, user_inputs):
        uidx = Variable(torch.LongTensor(user_inputs).cuda())
        members_embeds = self.userembeds(uidx)
        follow_embeds = self.followembeds(uidx)
        mf_embeds = torch.cat((members_embeds, follow_embeds), dim=1)
        at_wt = self.attention(mf_embeds)
        g_embeds_with_attention = torch.matmul(at_wt, members_embeds)
        user_finnal_vec = members_embeds + g_embeds_with_attention

        return user_finnal_vec
    
    # user forward
    def usr_forward(self, user_inputs, item_inputs):
        item_inputs_var = Variable(item_inputs).cuda()
